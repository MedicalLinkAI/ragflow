...
async def collect():
    ...
    if not redis_msg:
        return None, None
    logging.info(f"[DEBUG-COLLECT] got redis_msg, msg_id={redis_msg.get_msg_id()}")
    ...
    logging.info(f"[DEBUG-COLLECT] msg keys={list(msg.keys())}, task_type={msg.get('task_type','?')}, id={msg.get('id','?')}")
    canceled = False
    if msg.get("doc_id", "") in [GRAPH_RAPTOR_FAKE_DOC_ID, CANVAS_DEBUG_DOC_ID]:
        logging.info(f"[DEBUG-COLLECT] special doc_id branch")
        task = msg
        ...
        logging.info(f"[DEBUG-COLLECT] memory branch")
        _, task_obj = TaskService.get_by_id(msg["id"])
        task = task_obj.to_dict()
    else:
        logging.info(f"[DEBUG-COLLECT] normal branch, calling TaskService.get_task({msg['id']})")
        task = TaskService.get_task(msg["id"])
        logging.info(f"[DEBUG-COLLECT] get_task returned: {type(task)}, is_none={task is None}")
    ...
    for ck in chunks:
        ...
        if "row_positions" in ck:
            ...
            logging.info(
                ...
                (f" row[0]={rpi[0]} row[-1]={rpi[-1]}" if rpi else " (empty)")
            )
